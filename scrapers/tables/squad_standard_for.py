"""Squad standard for table scraper."""

from typing import Dict, List, Union
from sqlalchemy.orm import Session
from sqlalchemy import select
from database.models import TeamSquadStandardFor
from .base import BaseTableScraper
import logging

logger = logging.getLogger(__name__)

class SquadStandardForScraper(BaseTableScraper):
    """Scraper for squad standard for table."""

    def _find_squad_standard_table(self, tables_data: Dict[str, List[List[str]]]) -> Union[List[List[str]], None]:
        """Find the squad standard for table in the data."""
        for table_name, table_data in tables_data.items():
            if 'stats_squads_standard_for' in table_name.lower():
                return table_data
        logger.warning("Could not find squad standard for table")
        return None

    def _map_row_to_stats(self, row: List[str], stats_record: TeamSquadStandardFor, header_stats: List[str]) -> None:
    ...
    def save_to_db(self, tables_data: Dict[str, List[List[str]]], db: Session) -> None:
        """Save squad standard for table data to database."""
        try:
            ...
            for row in data_rows:
                ...
                existing_record = db.execute(
                    select(TeamSquadStandardFor).where(
                        TeamSquadStandardFor.team_id == team.id,
                        TeamSquadStandardFor.season == self.season,
                        TeamSquadStandardFor.competition_id == competition.id
                    )
                ).scalar_one_or_none()
            ...
            db.commit()
            logger.info(f"Successfully saved squad standard for stats for season {self.season}")

        except Exception as e:
            db.rollback()
            logger.error(f"Error saving squad standard for stats to database: {str(e)}")
            raise 