...
from database.models import TeamSquadKeeperFor
from .base import BaseTableScraper
import logging

logger = logging.getLogger(__name__)

class SquadKeeperForScraper(BaseTableScraper):
    """Scraper for squad keeper for table."""

    def _find_squad_keeper_table(self, tables_data: Dict[str, List[List[str]]]) -> Union[List[List[str]], None]:
        """Find the squad keeper for table in the data."""
        for table_name, table_data in tables_data.items():
            if 'stats_squads_keeper_for' in table_name.lower():
                return table_data
        logger.warning("Could not find squad keeper for table")
        return None

    def _map_row_to_stats(self, row: List[str], stats_record: TeamSquadKeeperFor, header_stats: List[str]) -> None:
    ...
    def save_to_db(self, tables_data: Dict[str, List[List[str]]], db: Session) -> None:
        """Save squad keeper for table data to database."""
        try:
            ...
            for row in data_rows:
                ...
                existing_record = db.execute(
                    select(TeamSquadKeeperFor).where(
                        TeamSquadKeeperFor.team_id == team.id,
                        TeamSquadKeeperFor.season == self.season,
                        TeamSquadKeeperFor.competition_id == competition.id
                    )
                ).scalar_one_or_none()
            ...
            db.commit()
            logger.info(f"Successfully saved squad keeper for stats for season {self.season}")

        except Exception as e:
            db.rollback()
            logger.error(f"Error saving squad keeper for stats to database: {str(e)}")
            raise 