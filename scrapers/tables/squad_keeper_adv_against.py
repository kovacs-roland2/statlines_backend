"""Squad keeper advanced against table scraper."""

from typing import Dict, List, Union
from sqlalchemy.orm import Session
from sqlalchemy import select
from database.models import TeamSquadKeeperAdvAgainst
from .base import BaseTableScraper
import logging

logger = logging.getLogger(__name__)

class SquadKeeperAdvAgainstScraper(BaseTableScraper):
    """Scraper for squad keeper advanced against table."""

    def _find_squad_keeper_adv_table(self, tables_data: Dict[str, List[List[str]]]) -> Union[List[List[str]], None]:
        """Find the squad keeper advanced against table in the data."""
        for table_name, table_data in tables_data.items():
            if 'stats_squads_keeper_adv_against' in table_name.lower():
                return table_data
        logger.warning("Could not find squad keeper advanced against table")
        return None

    def _map_row_to_stats(self, row: List[str], stats_record: TeamSquadKeeperAdvAgainst, header_stats: List[str]) -> None:
    ...
    def save_to_db(self, tables_data: Dict[str, List[List[str]]], db: Session) -> None:
        """Save squad keeper advanced against table data to database."""
        try:
            ...
            for row in data_rows:
                ...
                existing_record = db.execute(
                    select(TeamSquadKeeperAdvAgainst).where(
                        TeamSquadKeeperAdvAgainst.team_id == team.id,
                        TeamSquadKeeperAdvAgainst.season == self.season,
                        TeamSquadKeeperAdvAgainst.competition_id == competition.id
                    )
                ).scalar_one_or_none()
                ...
                if existing_record:
                    ...
                    record = TeamSquadKeeperAdvAgainst(
                        team_id=team.id,
            ...
            db.commit()
            logger.info(f"Successfully saved squad keeper advanced against stats for season {self.season}")

        except Exception as e:
            db.rollback()
            logger.error(f"Error saving squad keeper advanced against stats to database: {str(e)}")
            raise 